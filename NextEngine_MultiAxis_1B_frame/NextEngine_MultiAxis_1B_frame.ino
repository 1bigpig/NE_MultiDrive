...
uint8_t PICmem[256]={0x02,0x03,0x00,0x01,0x06,0x07,0x04,0x05,0x0A,0x0B,0x08,0x09,0x0E,0x0F,0xE9,0x0D,
  ...
  uint8_t         counter=1;            //persistant counter
...
/*
      respondByte[4]=0x04;
      respondByte[5] =(byte) (indexMark & 0xff);         //low byte
      respondByte[6] =(byte) ((indexMark >> 8) & 0xff);  //low word
      respondByte[7] =(byte) ((indexMark >> 16) & 0xff); //high word
      respondByte[8] =(byte) ((indexMark >> 24) & 0xff); //high word
      respondByte[9]=checkSum(6);
      respondByte[10]=0x00;
      respondByte[20]=10;    //number of bytes in response, index=1
*/
      break;
...
//      respondByte[5]=(byte) counter++;    //increase counter
 ...
 */
    if (counter>255) counter=0;
      ...
      if (counter++%2)
      {
              respondByte[5] =0x0F;                               //unknown byte
      }
      else
      {
              respondByte[5] =0xF0;                               //unknown byte
      }
//      respondByte[5] =PICmem[(byte) counter++];
//      respondByte[5] =0x00;                               //unknown byte
      ...
      if (firstPass)                  //First pass "Starting Motion" response
      {
        ...
        if (indexLoc>0) {             //We are not rotating 360, now only +/- 180 Degrees.  Index is right in the middle
          indexMark=1;
//        indexMark=25751;
        } else {
        indexMark=-1;      //Calculated 10' as zero
...
//80' is 206008 Native steps
//-40' is -103004 (-35' usable travel)
//+40 is 103004   (45' usable travel)
//index mark center is at level position of +57225
...
//          moveSteps=90129;     //-35'
        ...
        moveDegrees=(moveSteps*0.02307);      //convert navtive steps to OUR steps.  new conversion factor
